        ...
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_genre_sel == 0:
                    strsql_personnes_afficher = """SELECT t_personne.*, t_email.adresse_email AS nom_email, t_adresse.*
                                                        FROM t_personne
                    ...
                    strsql_personnes_afficher = """SELECT id_personne, prenom, nom, fk_adresse, fk_email  FROM t_personne WHERE id_personne = %(value_id_genre_selected)s"""

                    mc_afficher.execute(strsql_personnes_afficher, valeur_id_genre_selected_dictionnaire)
                else:
                    strsql_personnes_afficher = """SELECT id_personne, prenom, nom, fk_adresse, fk_email  FROM t_personne ORDER BY id_personne DESC"""
...
def personnes_ajouter_wtf():
    ...
    try:
        if request.method == "POST" and form.submit.data:
            ...
            strsql_insert_genre = """INSERT INTO t_personne (id_personne,nom,prenom,fk_email, fk_adresse) VALUES (NULL,%(value_intitule_personne)s,%(value_prenom_personne)s,%(value_email_fk)s,%(value_adresse_fk)s)"""
...
def personne_update_wtf():
    ...
    try:
        ...
        if form_update.validate_on_submit():
            ...
            str_sql_update_intitulegenre = """UPDATE t_personne SET nom = %(value_name_genre)s, prenom = %(value_prenom_pers)s, fk_email = %(value_update_email)s, fk_adresse = %(value_update_adresse)s WHERE id_personne = %(value_id_genre)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_genre = "SELECT id_personne, nom, prenom, fk_email, fk_adresse FROM t_personne " \
